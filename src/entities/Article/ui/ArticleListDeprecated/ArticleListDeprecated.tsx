...
import cl from './ArticleListDeprecated.module.scss';
...
const getSkeletons = (view: ArticleView) => {
    return (
        new Array(view === ArticleView.BIG ? 3 : 9)
            .fill(0)
            .map((item, index) => <ArticleListItemSkeletonDeprecated className={cl.card} view={view} key={index} />)
    );
};

export const ArticleListDeprecated = memo((props: ArticleListProps) => {
    const {
        className,
        articles,
        isLoading,
